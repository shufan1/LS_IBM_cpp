...
ControlVar::ControlVar(const std::string &configPath) : ControlVar() {
    ...
    if (json_object_object_get_ex(root, "ControlVar", &section)) {
        ...
        if (json_object_object_get_ex(section, "dt_man", &v)) dt_man = json_object_get_double(v);
        ...
        if (json_object_object_get_ex(section, "debug", &v)) debug = json_object_get_boolean(v);
        if (json_object_object_get_ex(section, "verbose", &v)) verbose = json_object_get_boolean(v);
        if (json_object_object_get_ex(section, "freezeLS", &v)) freezeLS = json_object_get_boolean(v);
        ...
        if (json_object_object_get_ex(section, "ksp_type_scalar", &v)) ksp_type_scalar = json_object_get_string(v);
        if (json_object_object_get_ex(section, "pc_type_scalar", &v)) pc_type_scalar = json_object_get_string(v);
    }
